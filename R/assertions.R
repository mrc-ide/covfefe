...
#------------------------------------------------
# is class covfefe_indiv
assert_covfefe_indiv <- function(x, name = deparse(substitute(x))) {
  if (!is.covfefe_indiv(x)) {
    stop(sprintf("%s must be of class 'covfefe_indiv'", name), call. = FALSE)
  }
  return(TRUE)
}
